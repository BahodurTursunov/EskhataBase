...
namespace Sungero.RecordManagement.Server
{
  partial class ActionItemExecutionTaskRouteHandlers
  {
    ...
    public virtual void StartBlock113(Sungero.Workflow.Server.Route.MonitoringStartBlockEventArguments e)
    {
      // Пустой обработчик нужен для корректной работы задач стартованных в схеме версии 4.
    }
    ...
    {
      // Пустой обработчик нужен для корректной работы задач стартованных в схеме версии 3.
    }
    ...
    {
      // Обработчик нужен для корректной работы задач стартованных в схеме версии 3.
      return false;
    }
    ...
    public virtual void EndBlock8(Sungero.RecordManagement.Server.ActionItemExecutionNotificationEndBlockEventArguments e)
    {
       ...
       */
      var startedSchemeVersion = Sungero.RecordManagement.ActionItemExecutionTask.TaskExtensions.GetStartedSchemeVersion(_obj);
      if (startedSchemeVersion < Sungero.RecordManagement.ActionItemExecutionTask.LayerSchemeVersions.V2)
        Functions.ActionItemExecutionTask.SynchronizeResultGroup(_obj);
    }
