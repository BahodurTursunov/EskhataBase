  ...
  partial class ActionItemExecutionTaskFunctions
  {
    ...
    public virtual bool CheckArioTasksStatus()
    {
      var predictionInfo = ActionItemPredictionInfos.GetAll()
        .Where(x => x.TaskId == _obj.Id && x.TaskType == RecordManagement.ActionItemPredictionInfo.TaskType.ActionItem)
        .FirstOrDefault();

      if (predictionInfo == null)
      {
        Logger.DebugFormat("ActionItemExecutionTask (ID={0}). WaitArioProcessingBlockResult. Action item prediction item not found",
                           _obj.Id);
        return true;
      }
      ...
      if (arioTaskInfo.State == SmartProcessing.PublicConstants.Module.ProcessingTaskStates.ErrorOccurred)
      {
        if (!string.IsNullOrEmpty(arioTaskInfo.ErrorMessage) || string.IsNullOrEmpty(arioTaskInfo.ResultJson))
          Logger.DebugFormat("ActionItemExecutionTask (ID={0}). WaitArioProcessingBlockResult. Error while getting result from Ario, arioTaskId={1}, errorMessage={2}",
                             _obj.Id, arioTaskInfo.Id, arioTaskInfo.ErrorMessage);
    ...
    {
      if (!Commons.PublicFunctions.Module.IsIntelligenceEnabled())
      {
        Logger.DebugFormat("ActionItemExecutionTask (ID={0}). WithDraftActionItem. Module license \"Intelligence\" not found.", task.Id);
        return false;
      }
      ...
      if (document == null)
      {
        Logger.DebugFormat("ActionItemExecutionTask (ID={0}). WithDraftActionItem. No document found", task.Id);
        return false;
      }
      if (!document.HasVersions)
      {
        Logger.DebugFormat("ActionItemExecutionTask (ID={0}). WithDraftActionItem. Document has no verisons", task.Id);
        return false;
      }

      var assistant = Functions.ActionItemExecutionTask.GetAIAssistantPreparingActionItemDrafts(task);
      if (assistant == null)
      {
        Logger.DebugFormat("ActionItemExecutionTask (ID={0}). WithDraftActionItem. AI assistant not found", task.Id);
        return false;
      }
      ...
      {
        Logger.DebugFormat("ActionItemExecutionTask (ID={0}). WithDraftActionItem. Assignee classifier ID not found, assistantId={1}",
                           task.Id, assistant.Id);
        return false;
      }
      
      if (assigneeClassifier.IsModelActive != Intelligence.AIManagersAssistantClassifiers.IsModelActive.Yes)
      {
        Logger.DebugFormat("ActionItemExecutionTask (ID={0}). WithDraftActionItem. Assignee classifier has no active models, assistantId={1}",
                           task.Id, assistant.Id);
        return false;
      }
    ...
    {
      return Intelligence.AIManagersAssistants.GetAll()
        .Where(x => Equals(x.Manager, _obj.Assignee) &&
    ...
    public virtual void AddDraftActionItemToParentAssignment(IActionItemExecutionAssignment assignment)
    {
      ...
      // Проверить, что проект подчиненного поручения создан для текущего исполнителя.
      if (!Equals(draftActionItem.AssignedBy, assignment.Performer))
        return;
    ...
    public virtual IActionItemExecutionTask CreateDraftActionItemExecutionTask(IEmployee performer)
    {
      ...
      draftActionItem.AssignedBy = _obj.Assignee;
      draftActionItem.StartedBy = _obj.Assignee;
      draftActionItem.Importance = _obj.Importance;
      draftActionItem.HasIndefiniteDeadline = _obj.HasIndefiniteDeadline == true;
      draftActionItem.Deadline = _obj.Deadline;
      draftActionItem.MaxDeadline = _obj.MaxDeadline;
      draftActionItem.ActiveText = ActionItemExecutionTasks.Resources.DefaultDraftActionItemActiveText;
      var assigneePersonalSettings = Docflow.PublicFunctions.PersonalSetting.GetPersonalSettings(_obj.Assignee);
