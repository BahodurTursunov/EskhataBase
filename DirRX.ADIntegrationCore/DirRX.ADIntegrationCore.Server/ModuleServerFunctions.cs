...
namespace DirRX.ADIntegrationCore.Server
{
  ...
  public class ModuleFunctions
  {
    ...
    public IEmployee GetEmployeeById(long id)
    {
    ...
    protected virtual void UpdatePersonProperties(ImportContext importContext, IPerson person, IExternalLink externalLink, JObject jEmployee, List<IExternalLink> changedExternalLinks)
    {
      ...
      if (importContext.NeedGetPersonFullNameFromDisplayName)
      {
        ...
        if (PersonFullName.TryParse((string)jEmployee["DisplayNameForExtractingPersonFullName"], out personFullName))
        {
          ...
          lastName = (string)jEmployee["DisplayNameForExtractingPersonFullName"];
      }
      else
      {
    ...
    /// <param name="changedExternalLinks">Список измененных ExternalLink-ов.</param>
    /// <param name="person">Персона.</param>
    protected virtual void UpdatePerson(ImportContext importContext, JObject jEmployee, IEmployee employee, List<IExternalLink> changedExternalLinks, out IPerson person)
    {
      var personExternalLink = GetPersonExternalLink(importContext, jEmployee);
      
      bool isCreated;
      person = GetOrCreatePerson(importContext, personExternalLink, out isCreated);
    ...
    protected virtual void HandleLoginCreated(ImportContext importContext, IEmployee employee, ILogin login, ImportResult importResult)
    {
      ...
      if (importContext.AuthenticationType == AuthenticationTypeConstants.Password)
      {
        ...
        LoginExtension.CreateNewPassword(login, out password);
      }
    ...
    protected virtual void UpdateLogin(ImportContext importContext, JObject jEmployee, IEmployee employee, ImportResult importResult, out ILogin login)
    {
      bool isCreated;
    ...
    /// <param name="login">Учетная запись.</param>
    /// <param name="person">Персона.</param>
    protected virtual void UpdateStatus(ImportContext importContext, JObject jEmployee, IEmployee employee, ILogin login, IPerson person)
    {
      ...
      {
        UpdateEnumRequisite(importContext, new Enumeration("Closed"), "Status", employee);
        UpdateEnumRequisite(importContext, new Enumeration("Closed"), "Status", person);
        UpdateEnumRequisite(importContext, new Enumeration("Closed"), "Status", login);
        HandleEmployeeClosed(employee, jEmployee);
        if (employee.State.IsChanged || person.State.IsChanged || login.State.IsChanged)
          AddToChangedEmployees(importContext, employee);
      }
      else
      {
        if (IsFillRequiredProperty(employee))
        {
          UpdateEnumRequisite(importContext, new Enumeration("Active"), "Status", employee);
          UpdateEnumRequisite(importContext, new Enumeration("Active"), "Status", person);
          UpdateEnumRequisite(importContext, new Enumeration("Active"), "Status", login);
    ...
    protected virtual void SendCredentialNotification(List<Credential> credentials, string notificationRole, string authenticationType)
    {
      if (credentials.Any())
      {
        string notificationText;
        if (authenticationType == AuthenticationTypeConstants.Windows)
        {
        ...
        var notificationPerformers = GetNotificationPerformers(notificationRole);
        SendNotice("Сгенерированы новые учетные записи", notificationText, notificationPerformers, null);
      }
    }
    ...
    protected virtual void SendErrorNotification(List<UnsavedInfo> unsavedEmployees, string notificationRole)
    {
      if (unsavedEmployees.Any())
      {
        ...
        ExtendedLogger.Error(unsavedEntitiesTextForLog);
        SendNotice("Ошибки синхронизации сотрудников", unsavedEntitiesText, notificationPerformers, null);
      }
    }
    ...
    protected virtual void SendDetermineDepartmentTask(List<EmployeeInfo> employeesWithoutDepartment, string notificationRole)
    {
      if (employeesWithoutDepartment.Any())
      {
        ...
        {
          var employee = GetEmployeeById(employeeInfo.Id);
          if (employee != null)
          {
        ...
        SendTask("Необходимо заполнить подразделение у сотрудников", taskText, notificationPerformers, attachments);
      }
    }
    ...
    /// </summary>
    protected virtual void SendDepartmentUndefinedNotification(List<EmployeeInfo> undefinedDepartmentEmployees, string notificationRole)
    {
      if (undefinedDepartmentEmployees.Any())
      {
        ...
        SendNotice("Ошибки синхронизации подразделений", notificationText, notificationPerformers, attachments);
      }
    }
    ...
    private void HandleImportError(Exception e, IEmployee employeeEntity, ImportResult result)
    {
      if (employeeEntity != null)
      {
        ...
        ExtendedLogger.ErrorFormat("Can't save entity with id {0} and name {1}. Reason: {2}.", employeeEntity.Id, employeeEntity.DisplayValue, e.Message);
      }
      else
        ExtendedLogger.Error(e);
    }
    ...
    /// <summary>
    /// Импортировать данные из <paramref name="importParamsJson" />.
    /// </summary>
    /// <param name="importParamsJson">Данные для импорта (json представление <see cref="ImportParams" />).</param>
    /// <returns>Результаты импорта (json представление <see cref="ImportResult" />).</returns>
    [Public(WebApiRequestType = RequestType.Post)]
    public virtual string ImportJson(string importParamsJson)
    {
      var importParams = JsonConvert.DeserializeObject<ImportParams>(importParamsJson);
      var importResult = Import(importParams);
      return JsonConvert.SerializeObject(importResult);
    }
    
    /// <summary>
    /// Обработать результаты импорта <paramref name="importResultsJson"/>.
    /// </summary>
    /// <param name="importResultsJson">Результаты импорта (json представление <see cref="ImportResult" />).</param>
    [Public(WebApiRequestType = RequestType.Post)]
    public virtual void ProcessImportResults(string importResultsJson)
    {
      try
      {
        var processedImportResults = JsonConvert.DeserializeObject<JObject>(importResultsJson);
        
        var credentials = processedImportResults["Credentials"].ToObject<List<Credential>>();
        var unsavedEmployees = processedImportResults["UnsavedEmployees"].ToObject<List<UnsavedInfo>>();
        var newEmployeesWithoutDepartment = processedImportResults["NewEmployeesWithoutDepartment"].ToObject<List<EmployeeInfo>>();
        var undefinedDepartmentEmployees = processedImportResults["UndefinedDepartmentEmployees"].ToObject<List<EmployeeInfo>>();
        var notificationRole = processedImportResults["NotificationRole"].Value<string>();
        var authenticationType = processedImportResults["AuthenticationType"].Value<string>();
        
        SendCredentialNotification(credentials, notificationRole, authenticationType);
        SendErrorNotification(unsavedEmployees, notificationRole);
        SendDetermineDepartmentTask(newEmployeesWithoutDepartment, notificationRole);
        SendDepartmentUndefinedNotification(undefinedDepartmentEmployees, notificationRole);
      }
      catch (Exception ex)
      {
        ExtendedLogger.Error(ex);
        throw;
      }
    }
